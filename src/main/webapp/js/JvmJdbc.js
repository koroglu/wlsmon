...
comboFiles.on('select',function(combo,record,index){
	...
	var panelFrame = Ext.create('Ext.Panel', {
		 ...
		 frame:true,
		 scrollable:true,
		 html: '<iframe id="reportframe" width="80%" height="100%" src="./iframe-jvmjdbc.html" onLoad="this.width=screen.width-230;this.height=screen.height;"></iframe>',
	});
...
Ext.getCmp('tabs').add({id:'jvmjdbc',items:[panel],title:'JVM Jdbc', border:false,frame:false, autoHeight:true, closable:true});
Ext.getCmp('tabs').setActiveTab('jvmjdbc');