...
comboFiles.on('select',function(combo,record,index){
	...
	panelFrame = Ext.create('Ext.Panel', {
		 ...
		 frame:true,
		 scrollable:true,
		 html: '<iframe id="reportframe" width="80%" height="100%" src="./iframe-hptop.html" onLoad="this.width=screen.width-230;this.height=screen.height;"></iframe>',
	});
...
Ext.getCmp('tabs').add({id:'hptop',items:[panel],title:'Hp Top Stats', border:false,frame:false, autoHeight:true, closable:true});
Ext.getCmp('tabs').setActiveTab('hptop');